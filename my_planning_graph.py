...
class PgNode_s(PgNode):
    ...
    def __init__(self, symbol: str, is_pos: bool):
        ...
        :param symbol: expr ( state expressions)
        :param is_pos: bool  (Tells if the state expression is possible or not)
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            node_a = PgNode_a(action)    # create an action PgNode
            if node_a.prenodes.issubset(self.s_levels[level]):  # Check if node_a parent's S nodes are indeed subset of S nodes
                action_nodes.append(node_a)  # Add the reachable action node to the list of action nodes
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_a)  # Adding the action node as child to its preceding state node
                    node_a.parents.add(node_s)   # Adding the state parent nodes of this action node

        self.a_levels.append(action_nodes)   # Construct the action level from reachable action nodes
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
              # Taking the effect nodes for its preceding action and adding them to the level's state nodes
            for node_s in node_a.effnodes:
               state_nodes.add(node_s)  # Add reachable effect node to the set of state nodes
               node_a.children.add(node_s)  # Adding the state node as child to its preceding action node
               node_s.parents.add(node_a) # Adding the action nodes as parents of this node


        self.s_levels.append(state_nodes) # Construct the state level from the reachable state nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Ensure node_a1's pos (add) effects aren't canceled by node_a2's neg (rem) effects
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # Ensure node_a2's pos (add) effects aren't canceled by node_a1's neg (rem) effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Ensure node_a1's pos (add) effects don't conflict with node_a2's neg (rem) preconditions
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True

        # Ensure node_a2's pos (add) effects don't conflict with node_a1's neg (rem) preconditions
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True

        # Ensure node_a1's neg (rem) effects don't conflict with node_a2's pos (add) preconditions
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True

        # Ensure node_a2's neg (rem) effects don't conflict with node_a1's pos (add) preconditions
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Ensure node_a1's preconditions are not mutually exclusive with node_a2's preconditions
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # Ensure two state literals don't have opposite polarity
        bool = (node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2.is_pos)
        return bool
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # Ensure node_s1's preconditions are not mutually exclusive with node_s2's preconditions
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    #state.show()
                    #print(goal, state.symbol, state.literal,  goal == state.symbol, level_sum, level)
                    a = state.symbol
                    if not state.is_pos:
                        a = expr('~{}'.format(state.symbol))
                    if goal == a:
                        goal_found = True
                        level_sum += level
                        break
